...
#   Matrix build:   ~10 min  (O(K²) assembly + O(K) Hurwitz zeta)
#   GenericSchur:   ~1-3 hours  (O(n³) at 2048-bit via GenericSchur)
#   ordschur loop:  ~6-12 hours  (50 × O(n²) BigFloat Givens + Sylvester)
#
# Usage:
#   julia --project --startup-file=no -t auto scripts/bigfloat_spectral_K1024.jl
#
...
# Ball vectors: P_j(e₁) = ℓ_j(1) · v_j with componentwise radii
proj_center     = Matrix{BigFloat}(undef, n, NUM_EIGS)
proj_radius     = Matrix{BigFloat}(undef, n, NUM_EIGS)
...
if isfile(CACHE_ELL)
    ...
    if haskey(ell_cache, :proj_center)
        proj_center .= ell_cache[:proj_center]
        proj_radius .= ell_cache[:proj_radius]
        @info "  Loaded projector ball vectors"
    else
        # Reconstruct from scalar × vector (backward compat)
        @info "  Reconstructing projector ball vectors from ℓ_j × v_j..."
        for j in 1:NUM_EIGS
            local vj_real = real.(q1_vectors[j])
            for k in 1:n
                proj_center[k, j] = ell_center[j] * vj_real[k]
                proj_radius[k, j] = setrounding(BigFloat, RoundUp) do
                    abs(ell_radius[j]) * abs(vj_real[k]) +
                        eps(BigFloat) * abs(proj_center[k, j])
                end
            end
        end
    end
    ...
    for j in 1:NUM_EIGS
        ...
        # 7. Projector ball vector: P_j(e₁) = ℓ_j(1) · v_j  (componentwise)
        #    center[k] = ℓ_j_center · Re(v_j[k])
        #    radius[k] = |ℓ_j_radius| · |Re(v_j[k])| + rounding
        local vj_real = real.(Q_ord[:, 1])
        for k in 1:n
            proj_center[k, j] = ell_center[j] * vj_real[k]
            proj_radius[k, j] = setrounding(BigFloat, RoundUp) do
                abs(ell_radius[j]) * abs(vj_real[k]) +
    ...
    Serialization.serialize(CACHE_ELL, Dict(
        ...
        :proj_center => proj_center, :proj_radius => proj_radius))
    @info "ℓ_j(1) + projector ball vectors cached to $CACHE_ELL"
end

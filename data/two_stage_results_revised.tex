...
%====================================================================
\section{Certified spectral enclosures for the Gauss--Kuzmin--Wirsing operator}
\label{sec:gkw-certified-enclosures}

This section reports a fully \emph{certified} numerical enclosure of the leading part of the spectrum of the
Gauss--Kuzmin--Wirsing (GKW) transfer operator.
The analytic setting (Hardy spaces, boundedness/compactness, and the explicit truncation error
\(\|L-L_K\|\le \varepsilon_K\)) is established in the previous section; here we focus on the numerical pipeline
and the concrete constants used in the experiment.

Our certification follows the abstract ``resolvent bridge'' strategy: we first produce resolvent bounds and
spectral separation certificates for a finite-dimensional discretization, and then lift them to the
infinite-dimensional operator using the tail bound \(\varepsilon_K\).
To reduce computational cost, we use a two-stage (coarse--fine) workflow: a coarse discretization isolates
spectral clusters and provides initial resolvent bounds, which are then propagated and sharpened at a finer
level.

\subsection{Refined bounds for $C_2$ by splitting}
\label{subsec:C2-splitting}

\begin{remark}[Refined bounds for $C_2$ by splitting]\label{rem:C2-splitting}
Write
\[
...
=2+\frac{2}{N-\tfrac12},
\]
hence, splitting the series at $N$ yields
\[
\|L\|_{H^2(D_1)\to H^2(D_{3/2})}
\le
\sum_{n=1}^{N-1}\frac{\sqrt{2n+1}}{(n-\tfrac12)^2}
\;+
\sqrt{\,2+\frac{2}{N-\tfrac12}\,}\,
\sum_{n\ge N}(n-\tfrac12)^{-3/2}.
\]
The tail is expressed via the Hurwitz zeta function:
\[
\sum_{n\ge N}(n-\tfrac12)^{-3/2}
=\zeta\!\left(\tfrac32,\,N-\tfrac12\right).
\]
Table~\ref{tab:C2-bounds} reports rigorous upper bounds for $C_2$ obtained from this splitting strategy,
computed with \texttt{Arb}; the maximum enclosure radius is bounded by $5\times 10^{-34}$.
\end{remark}
...
\hline
\end{tabular}
\caption{Refined rigorous upper bounds for $C_2$ as a function of the splitting parameter $N$.}
\label{tab:C2-bounds}
\end{table}

\subsection{Computational environment and parameter choices}
\label{subsec:gkw-env-params}
% Script 1: full_certification_50eigs.jl (resolvent certification, all 50 eigenvalues)
% Script 2: spectral_data_50eigs.jl (NK + ell_j(1) at K=256, BigFloat Schur)
...
\quad 1a & CertifScripts on full $A_{48}$ ($j = 1$--$15$) & $<\!1$\,min \\
...
\midrule
\multicolumn{3}{l}{\emph{Script 2: NK + spectral coefficients (\texttt{spectral\_data\_50eigs.jl})}} \\
\quad Schur & BigFloat Schur, $K\!=\!256$, 512-bit (GenericSchur) & ${\sim}\,20$\,min \\
\quad NK & Newton--Kantorovich ($35/50$ pass) & ${\sim}\,30$\,min \\
\quad ordschur & $\ell_j(\mathbf{1})$ via ordschur + Sylvester ($50$ eigs) & ${\sim}\,2$\,min \\
\midrule
\multicolumn{3}{l}{\emph{Script 3: $K\!=\!1024$ spectral (\texttt{bigfloat\_spectral\_K1024.jl})}} \\
\quad Build & ArbNumerics matrix, $K\!=\!1024$, 2048-bit & ${\sim}\,15$\,min \\
\quad Schur & $1025 \times 1025$ GenericSchur at 2048-bit & ${\sim}\,2.8$\,hr \\
\quad Quality & Residual + orthogonality norms & ${\sim}\,30$\,min \\
\quad ordschur & $50 \times$ ordschur\_ball + Sylvester + perturbation & ${\sim}\,37$\,min \\
\quad Total & & ${\sim}\,3.8$\,hr \\
\bottomrule
\end{tabular}
\end{table}
...
Circle samples & $256$ \\
Method ($j = 1$--$15$) & CertifScripts circle certification on full $A_{48}$ \\
...
For $j = 1$--$15$, CertifScripts circle certification on the full $A_{48}$;
for $j = 16$--$50$, block Schur at $K = 256$ with CertifScripts on $T_{22}$.}
\label{tab:two-stage-results}\\
\toprule
$j$ & $\hat\lambda_j$ & $K_{\mathrm{low}}$ & $\alpha_1$ & Circle $r$ & NK radius & Riesz proj.~err \\
\midrule
\endfirsthead
...
$M_\infty = \|R_{L_r}(z)\|$ is the certified resolvent bound
for the infinite-dimensional operator via Neumann perturbation:
$M_\infty \leq \|R_{A_{K_{\mathrm{low}}}}\| / (1 - \alpha_1)$.
The transfer columns give $\|R_{A_{K_{\mathrm{high}}}}\|$ via reverse perturbation
...
$r = 1$.  The first $50$ eigenvalues $\lambda_1, \ldots, \lambda_{50}$
(ordered by decreasing modulus) are simple: all $50$ excluding circles
$\Gamma_j \subset \rho(L_r)$ are certified via the two-stage resolvent method
(Table~\ref{tab:two-stage-results}).  For the first $35$ eigenvalues,
the Newton--Kantorovich argument at $K = 256$ yields tight eigenpair enclosures:
\begin{enumerate}

\item $\lambda_{1}^* \in B(1.000000000000,\; 7.40 \times 10^{-45})$, \quad $\|P_{L_r}(\Gamma_{1}) - P_{A_{256}}(\Gamma_{1})\| \leq 1.01 \times 10^{-41}$
\item $\lambda_{2}^* \in B(-0.303663002899,\; 2.33 \times 10^{-44})$, \quad $\|P_{L_r}(\Gamma_{2}) - P_{A_{256}}(\Gamma_{2})\| \leq 3.77 \times 10^{-41}$
\item $\lambda_{3}^* \in B(0.100884509293,\; 7.32 \times 10^{-44})$, \quad $\|P_{L_r}(\Gamma_{3}) - P_{A_{256}}(\Gamma_{3})\| \leq 1.23 \times 10^{-40}$
\item $\lambda_{4}^* \in B(-0.035496159022,\; 2.17 \times 10^{-43})$, \quad $\|P_{L_r}(\Gamma_{4}) - P_{A_{256}}(\Gamma_{4})\| \leq 3.73 \times 10^{-40}$
\item $\lambda_{5}^* \in B(0.012843790362,\; 6.22 \times 10^{-43})$, \quad $\|P_{L_r}(\Gamma_{5}) - P_{A_{256}}(\Gamma_{5})\| \leq 1.08 \times 10^{-39}$
\item $\lambda_{6}^* \in B(-0.004717777512,\; 1.75 \times 10^{-42})$, \quad $\|P_{L_r}(\Gamma_{6}) - P_{A_{256}}(\Gamma_{6})\| \leq 3.01 \times 10^{-39}$
\item $\lambda_{7}^* \in B(0.001748675124,\; 4.85 \times 10^{-42})$, \quad $\|P_{L_r}(\Gamma_{7}) - P_{A_{256}}(\Gamma_{7})\| \leq 8.29 \times 10^{-39}$
\item $\lambda_{8}^* \in B(-0.0006520209,\; 1.33 \times 10^{-41})$, \quad $\|P_{L_r}(\Gamma_{8}) - P_{A_{256}}(\Gamma_{8})\| \leq 2.29 \times 10^{-38}$
\item $\lambda_{9}^* \in B(0.0002441315,\; 3.63 \times 10^{-41})$, \quad $\|P_{L_r}(\Gamma_{9}) - P_{A_{256}}(\Gamma_{9})\| \leq 6.49 \times 10^{-38}$
\item $\lambda_{10}^* \in B(-0.00009168908,\; 9.85 \times 10^{-41})$, \quad $\|P_{L_r}(\Gamma_{10}) - P_{A_{256}}(\Gamma_{10})\| \leq 2.05 \times 10^{-37}$
\item $\lambda_{11}^* \in B(0.00003451655,\; 2.66 \times 10^{-40})$, \quad $\|P_{L_r}(\Gamma_{11}) - P_{A_{256}}(\Gamma_{11})\| \leq 9.48 \times 10^{-37}$
\item $\lambda_{12}^* \in B(-0.00001301770,\; 7.16 \times 10^{-40})$, \quad $\|P_{L_r}(\Gamma_{12}) - P_{A_{256}}(\Gamma_{12})\| \leq 2.34 \times 10^{-36}$
\item $\lambda_{13}^* \in B(0.000004916782,\; 1.92 \times 10^{-39})$, \quad $\|P_{L_r}(\Gamma_{13}) - P_{A_{256}}(\Gamma_{13})\| \leq 2.31 \times 10^{-36}$
\item $\lambda_{14}^* \in B(-0.000001859307,\; 5.15 \times 10^{-39})$, \quad $\|P_{L_r}(\Gamma_{14}) - P_{A_{256}}(\Gamma_{14})\| \leq 2.38 \times 10^{-36}$
\item $\lambda_{15}^* \in B(0.0000007038113,\; 1.38 \times 10^{-38})$, \quad $\|P_{L_r}(\Gamma_{15}) - P_{A_{256}}(\Gamma_{15})\| \leq 5.36 \times 10^{-36}$
\item $\lambda_{16}^* \in B(-0.0000002666413,\; 3.67 \times 10^{-38})$, \quad $\|P_{L_r}(\Gamma_{16}) - P_{A_{256}}(\Gamma_{16})\| \leq 1.34 \times 10^{-25}$
\item $\lambda_{17}^* \in B(0.0000001010906,\; 9.78 \times 10^{-38})$, \quad $\|P_{L_r}(\Gamma_{17}) - P_{A_{256}}(\Gamma_{17})\| \leq 2.33 \times 10^{-24}$
\item $\lambda_{18}^* \in B(-3.834970e-8,\; 2.60 \times 10^{-37})$, \quad $\|P_{L_r}(\Gamma_{18}) - P_{A_{256}}(\Gamma_{18})\| \leq 4.02 \times 10^{-23}$
\item $\lambda_{19}^* \in B(1.455614e-8,\; 6.91 \times 10^{-37})$, \quad $\|P_{L_r}(\Gamma_{19}) - P_{A_{256}}(\Gamma_{19})\| \leq 6.96 \times 10^{-22}$
\item $\lambda_{20}^* \in B(-5.527568e-9,\; 1.84 \times 10^{-36})$, \quad $\|P_{L_r}(\Gamma_{20}) - P_{A_{256}}(\Gamma_{20})\| \leq 1.20 \times 10^{-20}$
\item $\lambda_{21}^* \in B(2.099914e-9,\; 4.87 \times 10^{-36})$, \quad $\|P_{L_r}(\Gamma_{21}) - P_{A_{256}}(\Gamma_{21})\| \leq 2.08 \times 10^{-19}$
\item $\lambda_{22}^* \in B(-7.980458e-10,\; 1.29 \times 10^{-35})$, \quad $\|P_{L_r}(\Gamma_{22}) - P_{A_{256}}(\Gamma_{22})\| \leq 3.60 \times 10^{-18}$
\item $\lambda_{23}^* \in B(3.033863e-10,\; 3.41 \times 10^{-35})$, \quad $\|P_{L_r}(\Gamma_{23}) - P_{A_{256}}(\Gamma_{23})\| \leq 6.23 \times 10^{-17}$
\item $\lambda_{24}^* \in B(-1.153695e-10,\; 9.02 \times 10^{-35})$, \quad $\|P_{L_r}(\Gamma_{24}) - P_{A_{256}}(\Gamma_{24})\| \leq 1.08 \times 10^{-15}$
\item $\lambda_{25}^* \in B(4.388345e-11,\; 2.38 \times 10^{-34})$, \quad $\|P_{L_r}(\Gamma_{25}) - P_{A_{256}}(\Gamma_{25})\| \leq 1.87 \times 10^{-14}$
\item $\lambda_{26}^* \in B(-1.669605e-11,\; 6.29 \times 10^{-34})$, \quad $\|P_{L_r}(\Gamma_{26}) - P_{A_{256}}(\Gamma_{26})\| \leq 3.25 \times 10^{-13}$
\item $\lambda_{27}^* \in B(6.353612e-12,\; 1.66 \times 10^{-33})$, \quad $\|P_{L_r}(\Gamma_{27}) - P_{A_{256}}(\Gamma_{27})\| \leq 5.67 \times 10^{-12}$
\item $\lambda_{28}^* \in B(-2.418317e-12,\; 4.38 \times 10^{-33})$, \quad $\|P_{L_r}(\Gamma_{28}) - P_{A_{256}}(\Gamma_{28})\| \leq 1.01 \times 10^{-10}$
\item $\lambda_{29}^* \in B(9.206274e-13,\; 1.16 \times 10^{-32})$, \quad $\|P_{L_r}(\Gamma_{29}) - P_{A_{256}}(\Gamma_{29})\| \leq 1.87 \times 10^{-9}$
\item $\lambda_{30}^* \in B(-3.505309e-13,\; 3.06 \times 10^{-32})$, \quad $\|P_{L_r}(\Gamma_{30}) - P_{A_{256}}(\Gamma_{30})\| \leq 3.93 \times 10^{-8}$
\item $\lambda_{31}^* \in B(1.334857e-13,\; 8.08 \times 10^{-32})$, \quad $\|P_{L_r}(\Gamma_{31}) - P_{A_{256}}(\Gamma_{31})\| \leq 1.26 \times 10^{-6}$
\item $\lambda_{32}^* \in B(-5.083983e-14,\; 2.14 \times 10^{-31})$, \quad $\|P_{L_r}(\Gamma_{32}) - P_{A_{256}}(\Gamma_{32})\| \leq 1.00 \times 10^{-2}$
\item $\lambda_{33}^* \in B(1.936554e-14,\; 5.63 \times 10^{-31})$, \quad $\|P_{L_r}(\Gamma_{33}) - P_{A_{256}}(\Gamma_{33})\| \leq 9.86 \times 10^{-6}$
\item $\lambda_{34}^* \in B(-7.377470e-15,\; 1.52 \times 10^{-30})$, \quad $\|P_{L_r}(\Gamma_{34}) - P_{A_{256}}(\Gamma_{34})\| \leq 1.71 \times 10^{-4}$
\item $\lambda_{35}^* \in B(2.810825e-15,\; 5.55 \times 10^{-30})$, \quad $\|P_{L_r}(\Gamma_{35}) - P_{A_{256}}(\Gamma_{35})\| \leq 2.96 \times 10^{-3}$
...
Eigenvalues $36$--$50$ are certified to be simple (resolvent is finite on $\Gamma_j$)
but the NK radius is infinite at $K = 256$; at $K = 1024$ (2048-bit precision),
all $50$ eigenvalues have finite enclosures (Table~\ref{tab:spectral-coefficients}).

\medskip\noindent
\textbf{Method.}
For $j = 1, \ldots, 15$, Stage~1 certifies that $\Gamma_j \subset \rho(L_r)$ by verifying
$\alpha_1 = \varepsilon_{K_{\mathrm{low}}} \cdot \|R_{A_{K_{\mathrm{low}}}}(z)\|_{(\Gamma_j)} < 1$
via resolvent sampling with $256$ points on $\Gamma_j$, using the Galerkin matrix of size
$K_{\mathrm{low}} + 1 = 49$.
...
$\|R_{L_r}\|_{(\Gamma_{\mathrm{tail}})} \leq 6.06 \times 10^{41}$
...
Since all $50$ eigenvalues are certified to be simple (Theorem~\ref{thm:two-stage}),
...
\texttt{GenericSchur.jl}, achieving residual $\|A - QTQ'\|_2 \leq 2.82 \times 10^{-24}$
(before refinement) and orthogonality defect $\|Q'Q - I\|_2 \leq 1.75 \times 10^{-153}$.
The total Schur enclosure error is
$\|E\| = \|A_{\mathrm{true}} - QTQ'\|_2 \leq 4.96 \times 10^{-154}$.
For each eigenvalue $\lambda_j$, a BigFloat \texttt{ordschur} reorders the target eigenvalue
to position $(1,1)$. The reordering is realized as a product of unitary Givens rotations $(\widehat U)$;
in finite precision we track (i) the orthogonality defect ${\|I-\widehat U^*\widehat U\|}_2$ and
(ii) the reconstruction error $\|T-\widehat U\,\widetilde T\,\widehat U^*\|_2$.
The left eigenvector functional $\ell_j$ is then recovered via a Sylvester solve; the resulting projector
and resolvent bounds are transported from $\widetilde T$ back to $(T,A)$ by a two-step perturbation estimate.
More precisely, with $T = \bigl[\begin{smallmatrix} \lambda_j & T_{12} \\ 0 & T_{22}\end{smallmatrix}\bigr]$,
...
At $K = 1024$ with 2048-bit precision ($\approx 617$ decimal digits),
...
\caption{Rigorous spectral expansion coefficients $\ell_j(\mathbf{1})$ for the first $50$ eigenvalues.
Computed via BigFloat Schur decomposition (GenericSchur.jl) with Sylvester-based Riesz projector recovery.
At $K = 256$ (512-bit precision), $\|E\| \leq 4.96 \times 10^{-154}$;
at $K = 1024$ (2048-bit precision), $\|E\| \leq 5.72 \times 10^{-317}$.
All $50$ coefficients are sign-certified at $K = 1024$.
At $K = 256$, the first $35$ are certified; eigenvalues $36$--$50$ require $K = 1024$.}
\label{tab:spectral-coefficients}\\
\toprule
$j$ & $\hat\lambda_j$ & $\ell_j(\mathbf{1})$ & $\pm\;\delta$ ($K\!=\!256$) & $\pm\;\delta$ ($K\!=\!1024$) \\
\midrule
\endfirsthead
...
\midrule
\endhead
1 & $+1.000000000000e+00$ & $+8.32940370215781e-01$ & $1.58 \times 10^{-44}$ & $4.52 \times 10^{-316}$ \\
2 & $-3.036630028987e-01$ & $+4.51205768373325e-01$ & $5.33 \times 10^{-44}$ & $4.13 \times 10^{-315}$ \\
3 & $+1.008845092931e-01$ & $-2.01939874212917e-01$ & $1.76 \times 10^{-43}$ & $2.96 \times 10^{-314}$ \\
4 & $-3.549615902166e-02$ & $+8.74023929762093e-02$ & $5.45 \times 10^{-43}$ & $1.73 \times 10^{-313}$ \\
5 & $+1.284379036244e-02$ & $-3.69461611135829e-02$ & $1.61 \times 10^{-42}$ & $8.94 \times 10^{-313}$ \\
6 & $-4.717777511571e-03$ & $+1.53479541614426e-02$ & $4.62 \times 10^{-42}$ & $4.31 \times 10^{-312}$ \\
7 & $+1.748675124306e-03$ & $-6.29498041995732e-03$ & $1.30 \times 10^{-41}$ & $2.01 \times 10^{-311}$ \\
8 & $-6.520208583205e-04$ & $+2.55742091478648e-03$ & $3.62 \times 10^{-41}$ & $9.18 \times 10^{-311}$ \\
9 & $+2.441314655245e-04$ & $-1.03141519171637e-03$ & $9.97 \times 10^{-41}$ & $4.17 \times 10^{-310}$ \\
10 & $-9.168908376859e-05$ & $+4.13579574474742e-04$ & $2.73 \times 10^{-40}$ & $1.89 \times 10^{-309}$ \\
11 & $+3.451654616385e-05$ & $-1.65067361989440e-04$ & $7.42 \times 10^{-40}$ & $8.57 \times 10^{-309}$ \\
12 & $-1.301769787702e-05$ & $+6.56291175639149e-05$ & $2.01 \times 10^{-39}$ & $3.90 \times 10^{-308}$ \\
13 & $+4.916782302464e-06$ & $-2.60097167432892e-05$ & $5.41 \times 10^{-39}$ & $1.78 \times 10^{-307}$ \\
14 & $-1.859307351509e-06$ & $+1.02798813808483e-05$ & $1.45 \times 10^{-38}$ & $8.16 \times 10^{-307}$ \\
15 & $+7.038113430870e-07$ & $-4.05340297027246e-06$ & $3.90 \times 10^{-38}$ & $3.75 \times 10^{-306}$ \\
16 & $-2.666413434480e-07$ & $+1.59501183990722e-06$ & $1.04 \times 10^{-37}$ & $1.73 \times 10^{-305}$ \\
17 & $+1.010905532215e-07$ & $-6.26511706515095e-07$ & $2.78 \times 10^{-37}$ & $7.98 \times 10^{-305}$ \\
18 & $-3.834969795027e-08$ & $+2.45700158169347e-07$ & $7.40 \times 10^{-37}$ & $3.69 \times 10^{-304}$ \\
19 & $+1.455613838668e-08$ & $-9.62205238780812e-08$ & $1.97 \times 10^{-36}$ & $1.71 \times 10^{-303}$ \\
20 & $-5.527567937998e-09$ & $+3.76339163951678e-08$ & $5.23 \times 10^{-36}$ & $7.96 \times 10^{-303}$ \\
\midrule
21 & $+2.099913582973e-09$ & $-1.47026133408206e-08$ & $1.39 \times 10^{-35}$ & $3.70 \times 10^{-302}$ \\
22 & $-7.980457682720e-10$ & $+5.73798579791060e-09$ & $3.67 \times 10^{-35}$ & $1.72 \times 10^{-301}$ \\
23 & $+3.033862949099e-10$ & $-2.23724920262289e-09$ & $9.72 \times 10^{-35}$ & $8.04 \times 10^{-301}$ \\
24 & $-1.153695418145e-10$ & $+8.71553784016608e-10$ & $2.57 \times 10^{-34}$ & $3.75 \times 10^{-300}$ \\
25 & $+4.388345011398e-11$ & $-3.39257629140884e-10$ & $6.79 \times 10^{-34}$ & $1.75 \times 10^{-299}$ \\
26 & $-1.669605215117e-11$ & $+1.31961632720274e-10$ & $1.79 \times 10^{-33}$ & $8.21 \times 10^{-299}$ \\
27 & $+6.353612420983e-12$ & $-5.12946787975072e-11$ & $4.73 \times 10^{-33}$ & $3.84 \times 10^{-298}$ \\
28 & $-2.418316847595e-12$ & $+1.99262166328106e-11$ & $1.25 \times 10^{-32}$ & $1.80 \times 10^{-297}$ \\
29 & $+9.206274200576e-13$ & $-7.73614043768573e-12$ & $3.29 \times 10^{-32}$ & $8.43 \times 10^{-297}$ \\
30 & $-3.505309354594e-13$ & $+3.00184266520592e-12$ & $8.68 \times 10^{-32}$ & $3.96 \times 10^{-296}$ \\
31 & $+1.334857144144e-13$ & $-1.16420917757507e-12$ & $2.29 \times 10^{-31}$ & $1.86 \times 10^{-295}$ \\
32 & $-5.083983109741e-14$ & $+4.51302331569764e-13$ & $6.06 \times 10^{-31}$ & $8.72 \times 10^{-295}$ \\
33 & $+1.936554388309e-14$ & $-1.74867968492117e-13$ & $1.59 \times 10^{-30}$ & $4.10 \times 10^{-294}$ \\
34 & $-7.377470464407e-15$ & $+6.77283674309037e-14$ & $4.30 \times 10^{-30}$ & $1.92 \times 10^{-293}$ \\
35 & $+2.810824572326e-15$ & $-2.62215963077330e-14$ & $1.57 \times 10^{-29}$ & $9.05 \times 10^{-293}$ \\
\midrule
36 & $-1.071038650116e-15$ & $+1.01481167697676e-14$ & $\infty$ & $4.26 \times 10^{-292}$ \\
37 & $+4.081489263636e-16$ & $-3.92607356768238e-15$ & $\infty$ & $2.00 \times 10^{-291}$ \\
38 & $-1.555505453238e-16$ & $+1.51839998678424e-15$ & $\infty$ & $9.43 \times 10^{-291}$ \\
39 & $+5.928724623947e-17$ & $-5.87051535382839e-16$ & $\infty$ & $4.44 \times 10^{-290}$ \\
40 & $-2.259881129772e-17$ & $+2.26900510520210e-16$ & $\infty$ & $2.09 \times 10^{-289}$ \\
41 & $+8.614744429760e-18$ & $-8.76739032210825e-17$ & $\infty$ & $9.86 \times 10^{-289}$ \\
42 & $-3.284201293216e-18$ & $+3.38677844278173e-17$ & $\infty$ & $4.65 \times 10^{-288}$ \\
43 & $+1.252119972642e-18$ & $-1.30794758800253e-17$ & $\infty$ & $2.19 \times 10^{-287}$ \\
44 & $-4.774075699358e-19$ & $+5.04993929176121e-18$ & $\infty$ & $1.03 \times 10^{-286}$ \\
45 & $+1.820364445677e-19$ & $-1.94930160255074e-18$ & $\infty$ & $4.88 \times 10^{-286}$ \\
46 & $-6.941473812971e-20$ & $+7.52269541587240e-19$ & $\infty$ & $2.30 \times 10^{-285}$ \\
47 & $+2.647086022847e-20$ & $-2.90250965033080e-19$ & $\infty$ & $1.09 \times 10^{-284}$ \\
48 & $-1.009499834005e-20$ & $+1.11965328336240e-19$ & $\infty$ & $5.13 \times 10^{-284}$ \\
49 & $+3.850039422492e-21$ & $-4.31824051567987e-20$ & $\infty$ & $2.42 \times 10^{-283}$ \\
50 & $-1.468398058166e-21$ & $+1.66512542261763e-20$ & $\infty$ & $1.14 \times 10^{-282}$ \\
\bottomrule
\end{longtable}
...
\label{fig:eigenfunctions-11-20}
\end{figure}
...
\label{fig:eigenfunctions-21-30}
\end{figure}
...
\label{fig:eigenfunctions-31-40}
\end{figure}
...
\label{fig:eigenfunctions-41-50}
\end{figure}
...
\label{fig:eigenfunctions-1-10-overlay}
\end{figure}
...
\label{fig:eigenfunctions-11-20-overlay}
\end{figure}
...
\label{fig:eigenfunctions-21-30-overlay}
\end{figure}
...
\label{fig:eigenfunctions-31-40-overlay}
\end{figure}
...
\label{fig:eigenfunctions-41-50-overlay}
\end{figure}
...
\label{fig:eigenfunctions-all-overlay}
\end{figure}
...
(see Tables~\ref{tab:spectral-coefficients} and~\ref{tab:spectral-data-50}).
Eigenvectors at $K = 1024$ (2048-bit precision) achieve radii $\sim 10^{-316}$--$10^{-282}$.

\paragraph{$j = 1$: $\lambda_{1} = 1.000000e+00$, \quad $\ell_{1}(\mathbf{1}) = +8.329404e-01$, \quad $\|\hat{v}_{1} - v_{1}\|_2 \leq 6.81e-162$}
\begin{center}
{\footnotesize
...
\paragraph{$j = 2$: $\lambda_{2} = -3.036630e-01$, \quad $\ell_{2}(\mathbf{1}) = +4.512058e-01$, \quad $\|\hat{v}_{2} - v_{2}\|_2 \leq 6.23e-161$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 3$: $\lambda_{3} = 1.008845e-01$, \quad $\ell_{3}(\mathbf{1}) = -2.019399e-01$, \quad $\|\hat{v}_{3} - v_{3}\|_2 \leq 4.47e-160$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 4$: $\lambda_{4} = -3.549616e-02$, \quad $\ell_{4}(\mathbf{1}) = +8.740239e-02$, \quad $\|\hat{v}_{4} - v_{4}\|_2 \leq 2.61e-159$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 5$: $\lambda_{5} = 1.284379e-02$, \quad $\ell_{5}(\mathbf{1}) = -3.694616e-02$, \quad $\|\hat{v}_{5} - v_{5}\|_2 \leq 1.35e-158$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 6$: $\lambda_{6} = -4.717778e-03$, \quad $\ell_{6}(\mathbf{1}) = +1.534795e-02$, \quad $\|\hat{v}_{6} - v_{6}\|_2 \leq 6.52e-158$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 7$: $\lambda_{7} = 1.748675e-03$, \quad $\ell_{7}(\mathbf{1}) = -6.294980e-03$, \quad $\|\hat{v}_{7} - v_{7}\|_2 \leq 3.04e-157$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 8$: $\lambda_{8} = -6.520209e-04$, \quad $\ell_{8}(\mathbf{1}) = +2.557421e-03$, \quad $\|\hat{v}_{8} - v_{8}\|_2 \leq 1.39e-156$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 9$: $\lambda_{9} = 2.441315e-04$, \quad $\ell_{9}(\mathbf{1}) = -1.031415e-03$, \quad $\|\hat{v}_{9} - v_{9}\|_2 \leq 6.30e-156$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 10$: $\lambda_{10} = -9.168908e-05$, \quad $\ell_{10}(\mathbf{1}) = +4.135796e-04$, \quad $\|\hat{v}_{10} - v_{10}\|_2 \leq 2.86e-155$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 11$: $\lambda_{11} = 3.451655e-05$, \quad $\ell_{11}(\mathbf{1}) = -1.650674e-04$, \quad $\|\hat{v}_{11} - v_{11}\|_2 \leq 1.30e-154$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 12$: $\lambda_{12} = -1.301770e-05$, \quad $\ell_{12}(\mathbf{1}) = +6.562912e-05$, \quad $\|\hat{v}_{12} - v_{12}\|_2 \leq 5.90e-154$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 13$: $\lambda_{13} = 4.916782e-06$, \quad $\ell_{13}(\mathbf{1}) = -2.600972e-05$, \quad $\|\hat{v}_{13} - v_{13}\|_2 \leq 2.69e-153$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 14$: $\lambda_{14} = -1.859307e-06$, \quad $\ell_{14}(\mathbf{1}) = +1.027988e-05$, \quad $\|\hat{v}_{14} - v_{14}\|_2 \leq 1.23e-152$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 15$: $\lambda_{15} = 7.038113e-07$, \quad $\ell_{15}(\mathbf{1}) = -4.053403e-06$, \quad $\|\hat{v}_{15} - v_{15}\|_2 \leq 5.67e-152$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 16$: $\lambda_{16} = -2.666413e-07$, \quad $\ell_{16}(\mathbf{1}) = +1.595012e-06$, \quad $\|\hat{v}_{16} - v_{16}\|_2 \leq 2.61e-151$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 17$: $\lambda_{17} = 1.010906e-07$, \quad $\ell_{17}(\mathbf{1}) = -6.265117e-07$, \quad $\|\hat{v}_{17} - v_{17}\|_2 \leq 1.21e-150$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 18$: $\lambda_{18} = -3.834970e-08$, \quad $\ell_{18}(\mathbf{1}) = +2.457002e-07$, \quad $\|\hat{v}_{18} - v_{18}\|_2 \leq 5.59e-150$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 19$: $\lambda_{19} = 1.455614e-08$, \quad $\ell_{19}(\mathbf{1}) = -9.622052e-08$, \quad $\|\hat{v}_{19} - v_{19}\|_2 \leq 2.59e-149$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 20$: $\lambda_{20} = -5.527568e-09$, \quad $\ell_{20}(\mathbf{1}) = +3.763392e-08$, \quad $\|\hat{v}_{20} - v_{20}\|_2 \leq 1.20e-148$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 21$: $\lambda_{21} = 2.099914e-09$, \quad $\ell_{21}(\mathbf{1}) = -1.470261e-08$, \quad $\|\hat{v}_{21} - v_{21}\|_2 \leq 5.60e-148$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 22$: $\lambda_{22} = -7.980458e-10$, \quad $\ell_{22}(\mathbf{1}) = +5.737986e-09$, \quad $\|\hat{v}_{22} - v_{22}\|_2 \leq 2.61e-147$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 23$: $\lambda_{23} = 3.033863e-10$, \quad $\ell_{23}(\mathbf{1}) = -2.237249e-09$, \quad $\|\hat{v}_{23} - v_{23}\|_2 \leq 1.22e-146$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 24$: $\lambda_{24} = -1.153695e-10$, \quad $\ell_{24}(\mathbf{1}) = +8.715538e-10$, \quad $\|\hat{v}_{24} - v_{24}\|_2 \leq 5.68e-146$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 25$: $\lambda_{25} = 4.388345e-11$, \quad $\ell_{25}(\mathbf{1}) = -3.392576e-10$, \quad $\|\hat{v}_{25} - v_{25}\|_2 \leq 2.65e-145$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 26$: $\lambda_{26} = -1.669605e-11$, \quad $\ell_{26}(\mathbf{1}) = +1.319616e-10$, \quad $\|\hat{v}_{26} - v_{26}\|_2 \leq 1.24e-144$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 27$: $\lambda_{27} = 6.353612e-12$, \quad $\ell_{27}(\mathbf{1}) = -5.129468e-11$, \quad $\|\hat{v}_{27} - v_{27}\|_2 \leq 5.81e-144$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 28$: $\lambda_{28} = -2.418317e-12$, \quad $\ell_{28}(\mathbf{1}) = +1.992622e-11$, \quad $\|\hat{v}_{28} - v_{28}\|_2 \leq 2.72e-143$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 29$: $\lambda_{29} = 9.206274e-13$, \quad $\ell_{29}(\mathbf{1}) = -7.736140e-12$, \quad $\|\hat{v}_{29} - v_{29}\|_2 \leq 1.28e-142$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 30$: $\lambda_{30} = -3.505309e-13$, \quad $\ell_{30}(\mathbf{1}) = +3.001843e-12$, \quad $\|\hat{v}_{30} - v_{30}\|_2 \leq 5.98e-142$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 31$: $\lambda_{31} = 1.334857e-13$, \quad $\ell_{31}(\mathbf{1}) = -1.164209e-12$, \quad $\|\hat{v}_{31} - v_{31}\|_2 \leq 2.81e-141$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 32$: $\lambda_{32} = -5.083983e-14$, \quad $\ell_{32}(\mathbf{1}) = +4.513023e-13$, \quad $\|\hat{v}_{32} - v_{32}\|_2 \leq 1.32e-140$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 33$: $\lambda_{33} = 1.936554e-14$, \quad $\ell_{33}(\mathbf{1}) = -1.748680e-13$, \quad $\|\hat{v}_{33} - v_{33}\|_2 \leq 6.19e-140$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 34$: $\lambda_{34} = -7.377470e-15$, \quad $\ell_{34}(\mathbf{1}) = +6.772837e-14$, \quad $\|\hat{v}_{34} - v_{34}\|_2 \leq 2.91e-139$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 35$: $\lambda_{35} = 2.810825e-15$, \quad $\ell_{35}(\mathbf{1}) = -2.622160e-14$, \quad $\|\hat{v}_{35} - v_{35}\|_2 \leq 1.37e-138$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 36$: $\lambda_{36} = -1.071039e-15$, \quad $\ell_{36}(\mathbf{1}) = +1.014812e-14$, \quad $\|\hat{v}_{36} - v_{36}\|_2 \leq 6.44e-138$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 37$: $\lambda_{37} = 4.081489e-16$, \quad $\ell_{37}(\mathbf{1}) = -3.926074e-15$, \quad $\|\hat{v}_{37} - v_{37}\|_2 \leq 3.03e-137$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 38$: $\lambda_{38} = -1.555505e-16$, \quad $\ell_{38}(\mathbf{1}) = +1.518400e-15$, \quad $\|\hat{v}_{38} - v_{38}\|_2 \leq 1.43e-136$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 39$: $\lambda_{39} = 5.928725e-17$, \quad $\ell_{39}(\mathbf{1}) = -5.870515e-16$, \quad $\|\hat{v}_{39} - v_{39}\|_2 \leq 6.72e-136$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 40$: $\lambda_{40} = -2.259881e-17$, \quad $\ell_{40}(\mathbf{1}) = +2.269005e-16$, \quad $\|\hat{v}_{40} - v_{40}\|_2 \leq 3.17e-135$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 41$: $\lambda_{41} = 8.614744e-18$, \quad $\ell_{41}(\mathbf{1}) = -8.767390e-17$, \quad $\|\hat{v}_{41} - v_{41}\|_2 \leq 1.49e-134$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 42$: $\lambda_{42} = -3.284201e-18$, \quad $\ell_{42}(\mathbf{1}) = +3.386778e-17$, \quad $\|\hat{v}_{42} - v_{42}\|_2 \leq 7.03e-134$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 43$: $\lambda_{43} = 1.252120e-18$, \quad $\ell_{43}(\mathbf{1}) = -1.307948e-17$, \quad $\|\hat{v}_{43} - v_{43}\|_2 \leq 3.32e-133$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 44$: $\lambda_{44} = -4.774076e-19$, \quad $\ell_{44}(\mathbf{1}) = +5.049939e-18$, \quad $\|\hat{v}_{44} - v_{44}\|_2 \leq 1.56e-132$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 45$: $\lambda_{45} = 1.820364e-19$, \quad $\ell_{45}(\mathbf{1}) = -1.949302e-18$, \quad $\|\hat{v}_{45} - v_{45}\|_2 \leq 7.38e-132$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 46$: $\lambda_{46} = -6.941474e-20$, \quad $\ell_{46}(\mathbf{1}) = +7.522695e-19$, \quad $\|\hat{v}_{46} - v_{46}\|_2 \leq 3.48e-131$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 47$: $\lambda_{47} = 2.647086e-20$, \quad $\ell_{47}(\mathbf{1}) = -2.902510e-19$, \quad $\|\hat{v}_{47} - v_{47}\|_2 \leq 1.64e-130$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 48$: $\lambda_{48} = -1.009500e-20$, \quad $\ell_{48}(\mathbf{1}) = +1.119653e-19$, \quad $\|\hat{v}_{48} - v_{48}\|_2 \leq 7.76e-130$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 49$: $\lambda_{49} = 3.850039e-21$, \quad $\ell_{49}(\mathbf{1}) = -4.318241e-20$, \quad $\|\hat{v}_{49} - v_{49}\|_2 \leq 3.67e-129$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
\paragraph{$j = 50$: $\lambda_{50} = -1.468398e-21$, \quad $\ell_{50}(\mathbf{1}) = +1.665125e-20$, \quad $\|\hat{v}_{50} - v_{50}\|_2 \leq 1.73e-128$}
\begin{center}
{\footnotesize
\begin{tabular}{r@{\;\;}l@{\quad}r@{\;\;}l}
\toprule
...
$M_\infty = \|R_{L_r}\|$ on $\Gamma$ & $6.0617 \times 10^{41}$ \\
$\|Q_{50} \mathbf{1}\|_2$ & $6.1383 \times 10^{-21}$ \\
...
\begin{table}[ht]
\centering
\caption{Rigorous upper bounds on $\|R_{50}(n)\|_2 \leq \rho^{n+1} \cdot M_\infty \cdot \|Q_{50}\mathbf{1}\|_2$.}
...
Figure~\ref{fig:integrated-spectral} shows $F_n^{(j_0)}$ for $j_0 = 2,\ldots,6$
...
Figure~\ref{fig:schur-heatmap} shows the full $80 \times 80$ heatmap of
$\log_{10}|T_{ij}|$; Figure~\ref{fig:schur-rows} displays the row profiles
...
Right: off-diagonal row norms $\|T_{i, i+1:n}\|_2$ measuring the total
coupling strength from all higher modes into mode~$i$.}
\label{fig:schur-summary}
\end{figure}

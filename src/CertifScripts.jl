...
"""
    ...
    jobs::RemoteChannel(() -> Channel{Tuple{Int, ComplexF64}}(1024))
    results::RemoteChannel(() -> Channel{NamedTuple}(1024))

    Runs the svd certification routine for the matrix T-zI and returns a NamedTuple
    containing:
    - i the index of the job 
    - val the computed minimal singular value as a Ball
    - res the (linear algebraic) ℓ₂ bound on the resolvent as a Ball
    - hi_res, the certified upper bound on the resolvent
    - second_val, the second smallest singular value as a Ball
    - z, the point 
    - t, the time
...
"""
    Adaptive arcs
    Driver program run on the master process, 
    Inputs:
        - arcs, the list of arcs that need to be worked on 
        - cache, a dictionary that associates at each z the results of the computation
        - pending, a dictionary that associates to each job_id the arc of which we are waiting for the result
        - η: the relative size of the ℓ₂ ball for the resolvent estimate (enlargement)
        - check_interval: time at which we save a snapshot 
"""
...
"""
    Function that allows us to bound the resolvent norm for the original matrix from the resolvent 
    of the numerically obtained Schur decomposition
"""
...
"""
    Function that allows us to choose what is the best snapshot to recover the computation
"""
...
"""
    Cyclical save for saving snapshot A and B
"""
